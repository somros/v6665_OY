...
double Get_Tcorr(MSEBoxModel *bm, int sp, double current_temp, double *current_corr) {
  ...
  double temp_const_B = 0.0;
  
  if (bm->flagq10) {
    /* Now check the type of q10 correction that we are doing for this group */
    if (FunctGroupArray[sp].speciesParams[q10_method_id] == 1) {
      ...
      *current_corr = current_temp - opt_temp;
    ...
    } else if (FunctGroupArray[sp].speciesParams[q10_method_id] == 2) {
      ...
      /*
       if(sp == 16) {
       fprintf(bm->logFile, "First check: Day: %e, Species: %s, current_temp: %e, temp_const_A: %e, opt_temp: %e, temp_const_B: %e, step1: %e, step2: %e, step3: %e, step4: %e, ans: %e\n", bm->dayt, FunctGroupArray[sp].groupCode, current_temp, temp_const_A, opt_temp, temp_const_B, step1, step2, step3, step4, ans);
       }
       */
  ...
  /*
   if(sp == 16) {
   fprintf(bm->logFile, "Final check: Day: %e, Species: %s, current_temp: %e, ans: %e\n", bm->dayt, FunctGroupArray[sp].groupCode, current_temp, ans);
   }
   */
