...
//	Open_Manage_Output_Files(bm);
	...
	if (!bm->flag_fisheries_on) {
		...
		printf("ALBI Debug: rolling_cap_initialised allocation - dims: %d x %d, pointer: %p\n", 
       bm->K_num_tot_sp, 
	...
	for (sp = 0; sp < bm->K_num_tot_sp; sp++) {
		if (FunctGroupArray[sp].isFished == TRUE) {
			...
			for (fishery_id = 0; fishery_id < bm->K_num_fisheries; fishery_id++) {
			 ...
			 //fprintf(llogfp, "ALBI check 1: Group %s fleet %s has fishery_id: %d and has_F: %d, for K_num_Fisheries: %d\n", 
             //FunctGroupArray[sp].groupCode, FisheryArray[fishery_id].name, fishery_id, has_F, bm->K_num_fisheries); 
			  
				flagF = (int) (bm->SP_FISHERYprms[sp][fishery_id][flagF_id]);
			  
			  //fprintf(llogfp, "ALBI check 2: Group %s fleet %s has flagF_id %d and flagF %d\n", 
             //FunctGroupArray[sp].groupCode, FisheryArray[fishery_id].name, bm->SP_FISHERYprms[sp][fishery_id][flagF_id], flagF);
			  
				if (flagF) {
				  
				  //fprintf(llogfp, "ALBI check 3: we are in the if(flagF) bit\n");
				  
					has_F = 1;
				}
                
                //fprintf(llogfp, "ALBI check 4: Group %s fleet %s and now has_F is %d\n", 
                        //FunctGroupArray[sp].groupCode, FisheryArray[fishery_id].name, has_F);
