...
double Get_Fishing_Mortality(MSEBoxModel *bm, int sp, int chrt, int stage, int nf, double Biom, double li, int do_debug, FILE *llogfp) {
	...
	/*if ((sp==16 || sp==17) && ((int)(bm->dayt - 7) % 365 == 0)) {
	...
	//if (sp==16 && bm->dayt > 365) {
	  
	//fprintf(llogfp,"ALBI mFC2 - Time: %e %s %s box: %d, mpa_scale: %e\n",
         //bm->dayt, FisheryArray[nf].fisheryCode, FunctGroupArray[sp].groupCode, bm->current_box, mpa_scale);
