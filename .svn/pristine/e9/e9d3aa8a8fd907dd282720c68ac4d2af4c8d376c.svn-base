/*********************************************************************
    ...
    Created:        Mon Aug 24 11:46:42 EST 1998

    Author:         Jason Waring
                    CSIRO Marine Research

    Purpose:        Computes the geodetic distance between two points
		    on a spheroid, or compute the lat/long given a point
		    and distance.

		    A number of fomulations are used including
		    Robbin's and Sadano's.

    $Id: geodetic.c 2761 2011-07-01 04:35:25Z gor171 $
...
/** Inverse Robbins computation for distance, and azimuth on spheroid.
  ...
  * @return Distance.
  */
double inv_robbins(double x1, double y1, double z1,
	           ...
	           double a, double ecs)
{
	...
	double gh;
	  ...
	  double g2;
          sigp=sigp*sig;
          g2=sp1*sp1*eps;
          dum3=sigp*(h2*(4-7*h2)-3*g2*(1-7*h2))*8.3333333333333333e-3;
          dum2=dum1*dum3;
          dums=dums+dum2;
          if((fabs(dum2) > EABS) && (fabs(dum3) > EPROP)) {
            sigp=sigp*sig;
...
double a1 = (1+e22*sin2beta1)*(sin2beta1*cosphis+g*sinbeta1*sinphis);

   term1 = a1*(-e22*sinphis);
   term2 = m1*(-e24*phis + e24*sinphis*cosphis);
   term3 = a1*a1*((5*e4/8)*sinphis*cosphis);
   term4 = m1*m1*((11*e4/64)*phis - (13*e4/64)*sinphis*cosphis
	 - (e4/8)*phis*cos2phis + (5*e4/32)*sinphis*cos2phis*cosphis);
   term5 = a1*m1*((3*e4/8)*sinphis + (e4/4)*phis*cosphis
...
/*   az2 = atan(cosbeta0/(g*cosphi0 - sinbeta1*sinphi0)); */
   v = atan((sinphi0*saz)/(cosbeta1*cosphi0-sinbeta1*sinphi0*caz));
   term1 = -f*phis;
...
double b = a*(1-f);
   ...
   term4 = (sinbeta1*sinbeta2)*(sinbeta1*sinbeta2)*(-f2*sinphi*cosphi);
   term5 = m*m*(f5*phi + f5*sinphi*cosphi - f2*phi*phi/tanphi -
	   f4*sinphi*cosphi*cosphi*cosphi);
   term6 = sinbeta1*sinbeta2*m*(f2*phi*phi/sinphi + f2*sinphi*cosphi*cosphi);
...
double cosomega = cos(omega);
   ...
   term1 = f1*phi;
   term2 = sinbeta1*sinbeta2*(-f2*sinphi - f*f*phi*phi/sinphi);
   term3 = m*((-5*f2/2)*phi + (f2/2)*sinphi*cosphi+f*f*phi*phi/tanphi);
   v = c*(term1 + term2 + term3) + omega;
   sinv = sin(v);
   cosv = cos(v);

   az1 = atan2((sinv*cosbeta2),(sinbeta2*cosbeta1 - cosv*sinbeta1*cosbeta2));
   az2 = atan2((sinv*cosbeta1),(sinbeta2*cosbeta1*cosv - sinbeta1*cosbeta2));
		       ...
		       double *alon, double *alat)
{
   ...
   double cosdl = cos(dl);
   *alon = 2*M_PI - atan2(sin(dl),(cosplat*tan(elat) - sinplat*cosdl));
		       ...
		       double *elon, double *elat)
{
   ...
   *elon = atan2(sin(alon),(cosplat*tan(alat) - sinplat*cosalon)) + plon;
}
...
void main(int argc, char *argv[]) {
    ...
    double rdist = inv_robbins(x1, y1, 0.0, x2, y2, 0.0, a, e2);
    double sdist = inv_sodanos(x1, y1, x2, y2, a, e2);
    double az1 = inv_sodanos_angles(x1, y1, x2, y2, a, e2, &az2);
    double gcdist = gc_distance(x1, y1, x2, y2);

    printf("Robbins distance = %e\n", rdist);
    printf("Sodanos distance = %e\n", sdist);
    printf("Sodanos angles = %e %e\n", az1*180/M_PI, az2*180/M_PI);
    printf("Greate circle distance = %e\n", gcdist);

    sodanos(x1, y1, 315*M_PI/180.0, 1000000.0, a, e2, &x2, &y2);
    printf("Sodanos computed position = %e %e\n", x2*180.0/M_PI, y2*180.0/M_PI);

#if 0
    for (lon=-160.0; lon<=160.0; lon+=20.0) {
       for (lat=-80.0; lat<=80.0; lat+=20.0) {
	  fwd_spherical_rot(lon*M_PI/180.0, lat*M_PI/180.0,
			    plon, plat, &tlon, &tlat);
	  printf("%g %g %g %g", lon, lat,
			tlon*180/M_PI, tlat*180/M_PI);
	  inv_spherical_rot(tlon, tlat, plon, plat, &tlon, &tlat);
	  printf(" %g %g\n", tlon*180/M_PI, tlat*180/M_PI);
       }
    }
    ...
    lat = -25.53;
	  ...
	  printf("%g %g %g %g", lon, lat,
			tlon*180/M_PI, tlat*180/M_PI);
	  inv_spherical_rot(tlon+1*M_PI/180, tlat+1*M_PI/180, plon, plat, &tlon, &tlat);
	  printf(" %g %g\n", tlon*180/M_PI, tlat*180/M_PI);
#endif
