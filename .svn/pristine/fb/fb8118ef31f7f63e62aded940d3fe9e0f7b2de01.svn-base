/*
    ...
    Created:        Thu Sep 19 15:58:12 EST 1991
    ...
    $Id: drandom.c 2882 2011-08-11 05:56:44Z gor171 $
...
double drandom(double min, double max)
{
	...
	/* ran3() initialises itself automatically,
	 * so we don't ever need to do this
	 */

    initValue = 0;
    ret = (min + (max-min)*ran3(&initValue));
...
float ran1(int *idum)
{
	...
	if (*idum < 0 || iff == 0) {
		...
		for (j=1;j<=97;j++) {
			ix1=(IA1*ix1+IC1) % M1;
			ix2=(IA2*ix2+IC2) % M2;
			r[j]=(float)(((float)ix1+(float)ix2*RM2)*RM1);
		}
		*idum=1;
	}
	ix1=(IA1*ix1+IC1) % M1;
	...
	ix3=(IA3*ix3+IC3) % M3;
	j=1 + ((97*(int)ix3)/M3);
	if (j > 97 || j < 1) quit("RAN1: This cannot happen.");
	temp=r[j];
	r[j]=(float)(((float)ix1+(float)ix2*RM2)*RM1);
	return temp;
}
...
float ran2(long int *idum)
{
	...
	j= (int)(1 + 97.0*(int)iy/M);
	if (j > 97 || j < 1) quit("RAN2: This cannot happen.");
	iy=ir[j];
	*idum=(IA*(*idum)+IC) % M;
	ir[j]=(*idum);
	return (float) iy/M;
